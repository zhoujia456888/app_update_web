...
const selectedChannel = computed(() => {
  return channelOptions.value.find(
      (item) => item.channel_id === selectedChannelId.value,
  );
});
...
async function getChannelList() {
  try {
    ...
    const firstChannel = channelOptions.value[0];
    if (!selectedChannelId.value && firstChannel) {
      selectedChannelId.value = firstChannel.channel_id;
    }
  } catch (error: unknown) {
...
async function _submitAppInfo() {
  ...
  const payload: UploadAppFileCompleteReq = {
    ...
    file_name: uploadAppInfo.value.file_name || "",
...
async function _navigateToBack() {
